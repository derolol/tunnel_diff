...
class NIQE(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        total_niqe = torch.zeros(size=(B,)).float().to(input_tensor)

        for b in range(B):
            input_tensor_b = (input_tensor[b] + 1.) / 2.
            input_tensor_b = input_tensor_b.clone().detach().cpu()
            input_b = input_tensor_b.mul_(255).clamp_(0, 255).permute(1, 2, 0).type(torch.uint8).numpy()
            input_b = cv2.cvtColor(input_b, cv2.COLOR_RGB2BGR)

            total_niqe[b] = niqe(input_b)
