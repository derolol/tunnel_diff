...
def main() -> None:
    ...
    with open(log_csv_path, "w") as csvfile: 
        ...
        writer.writerow(["image_name", "precision", "accuracy", "recall", "f1_score", "iou"])
        ...
        for batch_idx, batch in enumerate(tqdm(val_dataloader)):
            ...
            batch["annotation"] = batch["annotation"].to(model_module.device)
        
            with torch.no_grad():

                rows, images = model_module.test_on_tunnel_defect_enhance(batch, batch_idx)
