...
from model.tunnel_diff.ldm.modules.diffusionmodules.util import make_beta_schedule
from model.tunnel_diff.cond_fn import Guidance
from model.tunnel_diff.utils.image import (
...
class SpacedSampler:
    ...
    def sample(
        ...
        if positive_prompt == "":
            positive_prompt = self.model.get_learned_conditioning([positive_prompt] * b)
        if negative_prompt == "":
            negative_prompt = self.model.get_learned_conditioning([negative_prompt] * b)
        cond = {
            # "c_latent": [self.model.apply_condition_encoder(cond_img)],
            "c_latent": cond_img,
            # "c_crossattn": [self.model.get_learned_conditioning([positive_prompt] * b)]
            "c_crossattn": [positive_prompt]
        }
        uncond = {
            # "c_latent": [self.model.apply_condition_encoder(cond_img)],
            "c_latent": cond_img,
            "c_crossattn": [negative_prompt]
        }
        for i, step in enumerate(iterator):
            ts = torch.full((b,), step, device=device, dtype=torch.long)
            index = torch.full_like(ts, fill_value=total_steps - i - 1)
