...
from model.zero_dce_plusplus.zero_dce_plusplus import ZeroDCEPlusPlus
...
class ZeroDCEPlusPlusModule(BaseModelModule):
    ...
    def __init__(self,
        ...
        self.model = ZeroDCEPlusPlus(scale_factor=self.scale_factor)
        
        self.loss_color = L_color()
        self.loss_spa = L_spa()
        self.loss_exp = L_exp(16)
        self.loss_tv = L_TV()
    ...
    def forward(self, batch):
        ...
        w_col = 1.0 # 1 s: 5.0
        w_tvA = 7.0 # 7.0 s: 1600.0

        input_enhanced, A = self.model(input_low)
        loss_tv = w_tvA * self.loss_tv(A)
        loss_spa = w_spa * torch.mean(self.loss_spa(input_low, input_enhanced))
        loss_col = w_col * torch.mean(self.loss_color(input_enhanced))
        loss_exp = w_exp * torch.mean(self.loss_exp(input_enhanced, E))
        
        # best_loss
        loss =  loss_tv + loss_spa + loss_col + loss_exp

        return loss, input_enhanced

    def training_step(self, batch, batch_idx):

        loss, input_enhanced = self(batch)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return input_enhanced

    def forward_enhance(self, batch, batch_idx):
        input_low = batch["hint"]
        input_enhance, A = self.model(input_low)
